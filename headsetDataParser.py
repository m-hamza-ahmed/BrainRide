...
class Parser:
    def __init__(self):
        ...
        # MODIFICATION BEGIN
        self.m_deviceName = "BRAINSENSE 17"
        self.m_deviceAddress = "00:18:E4:36:5C:EC"
        self.m_port = 1
        self.m_devicePassword = "1234"
        self.m_deviceFound = False
        ...
        while True:
            ...
            if len(nearbyDevices) > 0:
                for device in nearbyDevices:
                    if device[1] == self.m_deviceName:
                        print ("Found device!\nConnecting...")
                        self.m_deviceFound = True
                        break
            if self.m_deviceFound == True:
                break
            else:
        ...
        status = subprocess.call("bluetooth-agent " + self.m_devicePassword + " &", shell=True)
        print ("Bluetooth agent started...")
        # MODIFICATION END

        self.mindwaveMobileSocket = bluetooth.BluetoothSocket(bluetooth.RFCOMM)
        try:
            self.mindwaveMobileSocket.connect((self.m_deviceAddress, self.m_port))
        except bluetooth.btcommon.BluetoothError as error:
            print ("Could not connect: " + str(error) + "; Retrying in 5s...")
